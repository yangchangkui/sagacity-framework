...
public abstract class BaseService<M extends BaseMapper<T>,T> implements IService<T> {
     ...
     * @param bo 分页查询条件
     */
    public PageInfo<T> search(GenericBO<T> bo){
        PageHelper.startPage(bo.getPageNum(),bo.getPageSize());
        List<T> tList = mapper.search(bo);
     ...
     * @param bo 分页查询条件
     */
    public void export(GenericBO<T> bo) {
        // 设置导出数据默认值
        setExportDefaultParam(bo);
        PageInfo<T> pageInfo = search(bo);
        ExcelWriter excelWriter = ExcelUtils.getExcelWriter(null);
        excelWriter.write(pageInfo.getList());
        ...
        try {
            ...
            log.error("export getOutputStream error",e);
     ...
     *  @param bo 请求参数
     */
    private void setExportDefaultParam(GenericBO<T> bo) {
        // 第一页
        bo.setPageNum(bo.getPageNum() == 0 ? 1 : bo.getPageNum());
        // 默认导出条数
        bo.setPageSize(bo.getPageSize() > 5000 ? 5000 : bo.getPageSize());
    }
